#!/usr/bin/env python
# Created by "Thieu" at 16:18, 23/05/2025 ----------%
...
from waveletml import DataTransformer, GdWnnClassifier, CustomWaveletExpansionNetwork


def get_cross_val_score(X, y, cv=3):
    ## Train and test
    model = GdWnnClassifier(size_hidden=10, wavelet_fn="morlet", act_output=None,
                        epochs=50, batch_size=16, optim="Adam", optim_params=None,
...
def get_grid_search(X, y):
    ...
    ## Create a gridsearch
    model = GdWnnClassifier(epochs=50, valid_rate=0.1, seed=42, verbose=True, wnn_type=CustomWaveletExpansionNetwork)
